...
class Game:
    ...
    def _get_straight_flush_cards(self, all_player_cards: List[Card]) -> List[Card]:
        straight_cards = sorted(self._get_straight_cards(all_player_cards), key=lambda card: (card.value, card.suite))
        flush_cards = sorted(self._get_flush_cards(all_player_cards), key=lambda card: (card.value, card.suite))
